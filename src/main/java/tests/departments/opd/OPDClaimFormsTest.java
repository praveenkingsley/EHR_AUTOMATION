...
public class OPDClaimFormsTest extends TestBase {
    ...
    @Test(enabled = true, description = "Create patient in opd for claim forms validation")
    public void scheduleAdmissionInOpdForClaimForm() {
        ...
        try {
            ...
            if (bPatientFound) {
                ...
                m_assert.assertTrue(!Cls_Generic_Methods.isElementDisplayed(oPage_ClaimForms.button_claimForms),
                        "<b> OPD Claim forms button not visible when created cash bill </b> ");
            }
        } catch (Exception e) {
            e.printStackTrace();
            m_assert.assertFatal(" " + e);
    ...
    public void createDraftBillAndValidateClaimForms() {
        ...
        try {
            ...
            if (bPatientFound) {
                ...
                Cls_Generic_Methods.clickElement(oPage_Bills.input_amountPendingOpd);
                Cls_Generic_Methods.clearValuesInElement(oPage_Bills.input_amountPendingOpd);
                Cls_Generic_Methods.sendKeysIntoElement(oPage_Bills.input_amountPendingOpd, sPendingBalance);
    ...
    @Test(enabled = true, description = "Open all the OPD claim forms one by one and validate its opening in other window")
    public void validateOPDClaimForms() {
        oPage_PatientAppointmentDetails = new Page_PatientAppointmentDetails(driver);
        oPage_Bills = new Page_Bills(driver);
        oPage_ClaimForms = new Page_ClaimForms(driver);
        oPage_OPD = new Page_OPD(driver);
        ...
        String formsType[] = {"TPA Covering Letter", "Panel Covering Letter", "Raksha TPA Covering Letter", "DGEHS Panel Document",
                "Essentiality Certificate", "GIPSA Declaration", "Claim Form Part-A", "Claim Form Part-B", "Day Care Discharge Summary"};
        ...
        try {
            ...
            if (bPatientFound) {
                ...
                Cls_Generic_Methods.clickElement(oPage_ClaimForms.button_claimForms);
                m_assert.assertTrue(openClaimForm(oPage_ClaimForms.list_typeOfClaimForms, formsType[8]),
                        "<b>" + formsType[8] + " Opened </b> ");
