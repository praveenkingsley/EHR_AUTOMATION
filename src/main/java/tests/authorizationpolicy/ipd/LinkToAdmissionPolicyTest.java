...
public class LinkToAdmissionPolicyTest extends IPD_Policy{
    ...
    String sLinkToAdmissionPolicyComponent = "LINK TO ADMISSION (LINK TO ADMISSION)";


    @Test(description = "Validate Link OTs to Admissions access")
    public void validatePolicy_linkToAdmission() {
        ...
        try {
            ...
            setPolicy(sLinkToAdmissionPolicyComponent, false);
            ...
            boolean unlinkBtnDisplayed = Cls_Generic_Methods.waitForElementToBeDisplayed(oPage_OT.button_unlinkOt, 10);

            if(unlinkBtnDisplayed){
                Cls_Generic_Methods.clickElement(oPage_OT.button_unlinkOt);
            }

            boolean linkToAdmissionBtnDisplayed = Cls_Generic_Methods.waitForElementToBeDisplayed(oPage_OT.button_linkToAdmission, 10);
            m_assert.assertFalse(linkToAdmissionBtnDisplayed, "<font color='blue'>Validated " + sPolicyDescription + " is disabled for the user</font>");

            switchToAdmin();
            setPolicy(sLinkToAdmissionPolicyComponent, true);
            ...
            selectPatientFromOt();
            linkToAdmissionBtnDisplayed = Cls_Generic_Methods.waitForElementToBeDisplayed(oPage_OT.button_linkToAdmission, 10);
            m_assert.assertTrue(linkToAdmissionBtnDisplayed, "<font color='blue'>Validated " + sPolicyDescription + " is enabled for the user</font>");
